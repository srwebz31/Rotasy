        ...
        <div class="w-full px-5 py-5">
            <x-form action="{{ route('admin.notification.update', $notification) }}" method="put" has-file>
                ...
                <div class="flex justify-end">
                    <button type="submit" class="right-0 inline-block py-1 px-4 leading-loose bg-green-500 hover:bg-green-600 text-white font-semibold rounded-lg transition duration-200 text-sm" type="submit">{{ __('crud.general.udpate') }} {{ __('crud.admin.notifications.name') }}</button>
                </div>
            </x-form>
        </div>
    </div>
