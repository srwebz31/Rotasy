...
class ReferralResource extends Controller
{
        ...
        if (!empty($referral)) {
            ...
            if (config('constants.referral_amount') > 0) {
                ...
                if (config('constants.referral_count') > 0 && config('constants.referral_count') == $Referralcount) {
                    ...
                    (new TransactionResource())->referralCreditDebit(config('constants.referral_amount'), $referral->id, $type);
    ...
    {
        $ReferralHistory=ReferralEarning::where('referrer_id', $user_id)->where('type', $type)->get([
